...
void ppu_mode_oam()
{
  if (ppu_get_context()->line_ticks >= 80) LCDS_MODE_SET(MODE_XFER);
}

void ppu_mode_xfer()
{
  if (ppu_get_context()->line_ticks >= 80 + 172) LCDS_MODE_SET(MODE_HBLANK);
}
...
void increment_ly()
{
  ...
  if (lcd_get_context()->ly == lcd_get_context()->ly_compare) {
    ...
    if (LCDS_STAT_INT(SS_LYC)) cpu_request_interrupt(IT_LCD_STAT);
  } else {
    LCDS_LYC_SET(0);
  }
}
...
void ppu_mode_hblank()
{
  if (ppu_get_context()->line_ticks >= TICKS_PER_LINE) {
    ...
    if (lcd_get_context()->ly >= YRES) {
      ...
      if (LCDS_STAT_INT(SS_VBLANK)) cpu_request_interrupt(IT_LCD_STAT);
      ...
      if (frame_time < target_frame_time) delay((target_frame_time - frame_time));

      if (end - start_time >= 1000) {
        u32 fps = frame_count;
        start_time = end;
        frame_count = 0;
